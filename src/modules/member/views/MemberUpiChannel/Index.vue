...
import { deleteByIds, page } from '~/member/api/MemberUpiChannel.ts'
...
defineOptions({ name: 'member:member_upi_channel' })
        ...
        switch (formType) {
          ...
          case 'add':
            formRef.value.add().then((res: any) => {
              ...
              msg.alertError(err.response?.data?.message || err.message)
            })
            break
          // 修改
          case 'edit':
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => local('MemberUpiChannel.lable'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['member:member_upi_channel:create']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['member:member_upi_channel:delete']"
          type="danger"
          plain
