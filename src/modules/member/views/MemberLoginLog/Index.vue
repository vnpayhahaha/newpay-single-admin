...
import { deleteByIds, page } from '~/member/api/MemberLoginLog.ts'
...
defineOptions({ name: 'members:member_login_log' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('MemberLoginLog.lable'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['members:member_login_log:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['members:member_login_log:delete']"
          type="danger"
          plain
