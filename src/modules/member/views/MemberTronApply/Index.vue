...
import { deleteByIds, page } from '~/member/api/MemberTronApply.ts'
...
defineOptions({ name: 'members:member_tron_apply' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('MemberTronApply.lable'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['members:member_tron_apply:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['members:member_tron_apply:delete']"
          type="danger"
          plain
