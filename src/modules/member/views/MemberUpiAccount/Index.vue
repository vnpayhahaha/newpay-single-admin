...
import { deleteByIds, page } from '~/member/api/MemberUpiAccount.ts'
...
defineOptions({ name: 'member:member_upi_account' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => local('MemberUpiAccount.lable'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['member:member_upi_account:create']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['member:member_upi_account:delete']"
          type="danger"
          plain
